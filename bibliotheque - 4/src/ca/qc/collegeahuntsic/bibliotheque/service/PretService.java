...
public class PretService extends Service {
	...
	public PretService(PretDAO pretDAO, MembreDAO membreDAO, LivreDAO livreDAO,
	...
	public PretDTO get(int idPret) throws ServiceException {
		try {
			return getPretDAO().get(idPret);
		} catch (DAOException daoException) {
			throw new ServiceException(daoException);
		}
	}
	...
	public List<PretDTO> findByDatePret(Timestamp datePret)
			throws ServiceException {
		try {
	...
	public List<PretDTO> findByDateRetour(Timestamp dateRetour)
			throws ServiceException {
		try {
	...
	public void emprunter(MembreDTO membreDTO, LivreDTO livreDTO)
			throws ServiceException {
		try {
			// PretDTO unPretDTO = get(pretDTO.getIdPret());
			// if(unPretDTO != null) {
			// throw new ServiceException("Le prêt "
			// + pretDTO.getIdPret()
			// + " existe déjà");
			// }
			MembreDTO unMembreDTO = getMembreDAO().get(membreDTO.getIdMembre());
			if (unMembreDTO == null) {
				throw new ServiceException("Le membre "
						+ membreDTO.getIdMembre() + " n'existe pas");
			}
			LivreDTO unLivreDTO = getLivreDAO().get(livreDTO.getIdLivre());
			if (unLivreDTO == null) {
				throw new ServiceException("Le livre " + livreDTO.getIdLivre()
						+ " n'existe pas");
			}
			if (unMembreDTO.getNbPret() == unMembreDTO.getLimitePret()) {
				throw new ServiceException("Le membre " + unMembreDTO.getNom()
			...
			if (getReservationDAO().findByLivre(unLivreDTO) != null) {
				throw new ServiceException("Le livre " + unLivreDTO.getTitre()
			...
			long deuxSemaines = 14 * 24 * 60 * 60 * 1000;
			java.sql.Timestamp datePret = new Timestamp(
					System.currentTimeMillis());
			java.sql.Timestamp dateRetour = new java.sql.Timestamp(
					datePret.getTime() + deuxSemaines);
	 ...
	 *             quelqu'un d'autre, si le livre a été réservé ou s'il y a une
